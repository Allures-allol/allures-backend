...
```
# Публичный образ доступен на Docker Hub:
 
Docker Image: pavlovaalla88/allures-backend:latest

# Быстрый запуск (используя образ с Docker Hub):
...
# URLs микросервисов (если используется как монолит — можно указывать локальный адрес)
PRODUCT_SERVICE_URL=http://localhost:8000
SALES_SERVICE_URL=http://localhost:8001
...

# OpenAI ключ (для рекомендаций)
OPENAI_API_KEY=sk-...
🛠 Создание образа вручную (локально)
Если ты хочешь пересобрать образ:

docker build -t pavlovaalla88/allures-backend:latest .
docker push pavlovaalla88/allures-backend:latest
